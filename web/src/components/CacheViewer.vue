          ...
          <div class="storage-list">
            ...
            >
              <div class="item-header" @click="toggleItem(item.key)">
                ...
                <i :class="expandedItems[item.key] ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"></i>
              </div>
        ...
        <div class="storage-section">
          ...
          <div class="storage-list">
            <div
              ...
              class="storage-item"
            >
              <div class="item-header" @click="toggleItem(item.key + '_session')">
                <span class="item-key">{{ item.key }}</span>
                <span class="item-size">{{ item.size }}</span>
                <i :class="expandedItems[item.key + '_session'] ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"></i>
              </div>
              
              <div v-if="expandedItems[item.key + '_session']]" class="item-content">
                <pre class="json-viewer">{{ formatJson(item.value) }}</pre>
              </div>
            </div>
          ...
          <div class="storage-list">
            ...
            >
              <div class="item-header" @click="toggleItem(key + '_vuex')">
                ...
                <i :class="expandedItems[key + '_vuex'] ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"></i>
              </div>
            ...
            <div v-if="!vuexStore || Object.keys(vuexStore).length === 0" class="empty-state">
              ...
              <el-card class="summary-card">
                ...
                <div class="card-content">
                  <div class="stat-item">
                    ...
                    <span class="stat-value">{{ localStorageData.length }}</span>
                  </div>
                  <div class="stat-item">
                    <span class="stat-label">总大小:</span>
                    <span class="stat-value">{{ getTotalLocalStorageSize() }}</span>
                  </div>
                </div>
              </el-card>
            </el-col>
            ...
            <el-col :span="8">
              <el-card class="summary-card">
                ...
                <div class="card-content">
                  <div class="stat-item">
                    <span class="stat-label">项目数:</span>
                    <span class="stat-value">{{ sessionStorageData.length }}</span>
                  </div>
                  <div class="stat-item">
                    <span class="stat-label">总大小:</span>
                    <span class="stat-value">{{ getTotalSessionStorageSize() }}</span>
                  </div>
                </div>
              </el-card>
            </el-col>
              ...
              <el-card class="summary-card">
                ...
                <div class="card-content">
                  <div class="stat-item">
                    ...
                    <span class="stat-value">{{ vuexStore ? Object.keys(vuexStore).length : 0 }}</span>
                  </div>
                  <div class="stat-item">
                    <span class="stat-label">总大小:</span>
...
export default {
  ...
  computed: {
    ...
    filteredSessionStorage() {
      ...
      return this.sessionStorageData.filter(item =>
        item.key.toLowerCase().includes(this.sessionStorageSearch.toLowerCase())
      )
    },
    filteredVuex() {
  ...
  methods: {
    ...
    loadSessionStorage() {
      ...
      this.sessionStorageData = data.sort((a, b) => a.key.localeCompare(b.key))
    },
    loadVuex() {
      try {
    ...
    formatJson(data) {
      try {
        return JSON.stringify(typeof data === 'object' ? data : JSON.parse(data), null, 2)
      } catch (e) {
        return data
      }
    },
