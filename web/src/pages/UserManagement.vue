  ...
  <div class="user-management-container">
    <div class="header">
      ...
      <div class="header-actions">
        ...
        <el-button type="success" @click="goToMindMap">
          跳转到思维导图
        </el-button>
    ...
    <div class="content">
      <el-table :data="users" style="width: 100%">
        ...
        <el-table-column prop="mindMapPermission" label="导图权限" width="100">
          <template slot-scope="scope">
            <el-tag
              :type="scope.row.mindMapPermission === 1 ? 'success' : 'warning'"
            >
    ...
    methods: {
        ...
        if (this.$store.state.supabaseEnabled) {
          ...
          try {
            ...
            if (error) {
              ...
              // 回退到本地存储
              this.users = [...this.$store.state.users].sort(
                (a, b) => b.id - a.id
              )
            } else {
      ...
      toggleAdminStatus(user) {
        ...
        this.$message.success(
          `用户 ${user.username} 管理员权限已${user.isAdmin ? '取消' : '设置'}`
        )
        this.loadUsers() // 重新加载用户列表
      },

      async toggleMindMapPermission(user) {
        console.log('Toggle Mind Map Permission:', {
          user,
          currentPermission: user.mindMapPermission
        })
        ...
        if (this.$store.state.supabaseEnabled) {
          ...
          try {
            ...
            console.log('Updating user permission:', {
              userId: user.id,
              oldPermission: user.mindMapPermission,
              newPermission
            })

            const updatedUser = await userApi.updateMindMapPermission(
              user.id,
              newPermission
            )
          ...
          // 使用本地存储更新用户权限
          const newPermission = user.mindMapPermission === 1 ? 0 : 1
          console.log('Updating local user permission:', {
            userId: user.id,
            oldPermission: user.mindMapPermission,
            newPermission
          })
          ...
          this.$message.success(
            `用户 ${user.username} 导图权限已${permissionText}`
          )
          this.loadUsers() // 重新加载用户列表
        }
      },
