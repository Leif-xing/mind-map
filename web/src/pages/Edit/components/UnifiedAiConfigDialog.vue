    ...
    methods: {
      ...
      initConfig() {
        ...
        const currentProvider =
          this.aiSystem.providers[currentProviderKey] || {}
          ...
          if (valid) {
            ...
            try {
              ...
              if (this.existingConfigId) {
                // 如果是编辑现有配置
                result = await aiConfigApi.updateAiProviderConfig(
      ...
      async updateApiKey() {
        ...
        try {
          ...
          try {
            configsToUpdate =
              await aiConfigApi.getAiProviderConfigsByEndpoint(apiEndpoint)
          } catch (error) {
          ...
          this.$message.success(
            `成功更新了 ${configsToUpdate.length} 个配置的API密钥`
          )
        ...
        this.$nextTick(() => {
          if (this.visible) {
            const dialogHeaderEl = document.querySelector(
              '.unifiedAiConfigDialog .el-dialog__header'
            )
            const dragDom = document.querySelector(
              '.unifiedAiConfigDialog .el-dialog'
            )
      ...
      getStatusTitle() {
        const hasConfig =
          this.config.key && this.config.model && this.config.api
