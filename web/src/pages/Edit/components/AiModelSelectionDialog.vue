  ...
  >
    <div class="modelSelectionContent">
      ...
      <div v-else class="modelGrid">
        ...
        >
          <div class="modelInfo">
            <h3 class="providerName">{{ model.providerName || model.provider_name }}</h3>
            <p class="modelName">模型: {{ model.modelName || model.model_name }}</p>
            <p class="apiEndpoint" v-if="showApiEndpoint">接口: {{ model.apiEndpoint || model.api_endpoint }}</p>
            <el-tag 
  ...
  methods: {
    ...
    async selectModel(model) {
      try {
        ...
        this.$message.success(`已选择模型: ${model.providerName || model.provider_name} - ${model.modelName || model.model_name}`)
        this.visible = false
