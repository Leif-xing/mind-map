...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.$bus.$on(
        'triggerShiftWForRootNode',
        this.handleTriggerShiftWForRootNode
      )
    ...
    methods: {
      ...
      async initializeShortcutSystem() {
        try {
          ...
          console.error(
            'Edit component: Failed to initialize shortcut system:',
            error
          )
        }
      },
      ...
      toggleNodeNumberingFromShortcut(node, level, index) {
        ...
        const numberIconIndex = iconList.findIndex(item =>
          item.startsWith('number-')
        )
        ...
        if (numberIconIndex !== -1) {
          ...
          // 添加新的编号图标
          const numberingIcon = this.generateNumberingIconFromShortcut(
            level,
            index
          )
          if (numberingIcon) {
      ...
      toChineseNumberFromShortcut(num) {
        ...
        if (num < 100) {
          ...
          const ones = num % 10
          return (
            chineseDigits[tens] + '十' + (ones === 0 ? '' : chineseDigits[ones])
          )
        }
      ...
      setupNumberingUpdateListener() {
        ...
        this.handleExecCommand = (commandName, ...args) => {
          ...
          if (
            ...
            commandName === 'REMOVE_CURRENT_NODE' ||
            commandName === 'CUT_NODE'
          ) {
          ...
          // 节点位置变换相关命令
          if (
            commandName === 'MOVE_NODE_TO' ||
            commandName === 'UP_NODE' ||
            commandName === 'DOWN_NODE' ||
            commandName === 'INSERT_NODE'
          ) {
            this.triggerNumberingUpdate()
          }
        }
        ...
        if (children.length > 0) {
          ...
          if (numberedChildren.length > 0) {
            numberedChildren.forEach((child, index) => {
              ...
              if (numberIconIndex !== -1) {
                // 生成新的编号图标
                const newNumberingIcon = this.generateNumberingIconFromShortcut(
                  currentLevel,
                  index
                )
                if (newNumberingIcon) {
      ...
      addNodeNumbering(node, level, index) {
        ...
        const numberedText = this.addNumberingPrefix(
          currentText,
          numberingConfig
        )
      ...
      hasNumberingPrefix(text) {
        return /<span[^>]*style="[^"]*background-color:[^"]*"[^>]*>.*?<\/span>/.test(
          text
        )
      },
      ...
      removeNumberingPrefix(text) {
        // 移除HTML格式的圆形编号标记
        return text.replace(
          /<span[^>]*style="[^"]*background-color:[^"]*"[^>]*>.*?<\/span>\s*/i,
          ''
        )
      },
      ...
      handleToggleTodoCheckbox() {
        ...
        if (hasRootNode && activeNodes.length === 1) {
          ...
          if (selectedNode.children && selectedNode.children.length > 0) {
            // 检查所有直接子节点及其后代是否有复选框
            const allChildrenHaveCheckbox =
              this.checkAllDescendantsOfChildrenHaveCheckbox(selectedNode, [
                'checkbox_unchecked',
                'checkbox_success',
                'checkbox_failed'
              ])
            ...
            selectedNode.children.forEach(child => {
              this.toggleCheckboxForAllDescendants(
                child,
                false,
                !allChildrenHaveCheckbox
              )
            })
          }
        } else {
      ...
      toggleCheckboxForSingleNode(node) {
        ...
        const existingCheckboxIndex = iconList.findIndex(item =>
          checkboxIcons.includes(item)
        )
      ...
      async handleLoadMindMap(mindMapId) {
        try {
          ...
          // 从缓存中获取思维导图内容
          const cachedContent = await this.$store.dispatch(
            'getMindMapContent',
            mindMapId
          )
          ...
          if (cachedContent) {
            ...
            // 提示用户
            const mindMap = this.$store.state.localMindMaps.find(
              m => m.id === mindMapId
            )
            if (mindMap) {
