  ...
  <Sidebar ref="sidebar" :title="$t('tagManager.title')">
          ...
          <div v-if="filteredTags.length === 0 && !loading" class="emptyState">
            ...
            <p>
              {{
                searchKeyword
                  ? '没有找到匹配的标签'
                  : '暂无标签，点击上方按钮创建标签。创建后可双击标签快速添加到思维导图'
              }}
            </p>
          </div>
          <div v-else class="tagsList">
            <div
              ...
              :title="
                isProcessing
                  ? '处理中...'
                  : isTagUsedInCurrentMindMap(tag.id)
                    ? '双击移除标签'
                    : '双击添加标签到当前思维导图'
              "
            >
    ...
    >
      ...
      >
        ...
        {{
          isTagUsedInCurrentMindMap(contextMenu.tag?.id)
            ? '移除标签'
            : '添加标签'
        }}
      </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.$bus.$on(
        'mindmap-tag-association-changed',
        this.handleMindmapTagAssociationChanged
      )
    ...
    methods: {
      ...
      async loadCurrentMindMapTags() {
        ...
        try {
          // 先尝试从缓存获取
          const cachedTags = TagCacheManager.getMindMapTags(
      ...
      async handleDoubleClickTag(tag) {
        ...
        try {
          ...
          if (isUsed) {
            ...
            this.currentMindMapTags = TagCacheManager.getMindMapTags(
              this.currentMindMapId
            )
            ...
            tagApi
              ...
              .catch(error => {
                // 如果后台同步失败，回滚UI更改
                TagCacheManager.removeTagFromMindMap(
                  this.currentMindMapId,
                  tag.id
                )
                this.currentMindMapTags = TagCacheManager.getMindMapTags(
      ...
      handleTagSuccess(tagData) {
        ...
        if (tagData) {
          if (this.editingTag) {
            ...
            this.$nextTick(() => {
              if (this.currentMindMapId) {
                const newTags = TagCacheManager.getMindMapTags(
                  this.currentMindMapId
                )
      ...
      async refreshAllTagData() {
        try {
          ...
          const mindmapTagIds = await tagApi.getAllMindMapTagRelations(
            this.currentUser.id
          )
