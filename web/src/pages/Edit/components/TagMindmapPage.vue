    ...
    <div class="page-header">
      ...
      <div class="header-right">
        ...
        <el-button-group class="action-buttons">
          <el-button icon="el-icon-setting" @click="openSettings">
            设置
          </el-button>
        </el-button-group>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      userTags() {
        // 🔥 使用响应式数据，确保UI能够更新
        return Object.keys(this.cachedUserTags).length > 0
          ? this.cachedUserTags
          : TagCacheManager.getUserTags()
      },
      ...
      mindMapTagMapping() {
        ...
        if (
          ...
          JSON.stringify(cacheData) !== JSON.stringify(latestData)
        ) {
          return latestData
        }
        return cacheData
    ...
    created() {
      ...
      this.$bus.$on(
        'tag-statistics-update-needed',
        this.handleTagStatisticsUpdate
      )
      this.$bus.$on('tag-created', this.handleTagCreated)
      this.$bus.$on(
        'tag-mindmap-association-changed',
        this.handleTagMindmapAssociationChanged
      )
    },
    mounted() {
      // 页面加载完成后再次更新缓存
      ...
      // 注册导图管理页面的快捷键
      this.registerMindmapManagerShortcuts()
    },
    beforeDestroy() {
      ...
      // 清理导图管理页面的快捷键
      this.unregisterMindmapManagerShortcuts()
    },
    methods: {
      // 清理标题，移除HTML标签和多余字符
      cleanTitle(title) {
            ...
            if (cachedContent) {
              ...
              try {
                if (typeof cachedContent === 'string') {
                  ...
                  parsedTitle =
                    parsedContent.root?.data?.text ||
                    parsedContent.title ||
                    parsedContent.name ||
                    '未命名思维导图'
                } else if (cachedContent) {
                  parsedTitle =
                    cachedContent.root?.data?.text ||
                    cachedContent.title ||
                    cachedContent.name ||
      ...
      getUntaggedMindMaps() {
        try {
          ...
          const taggedIds = Object.keys(tagMappings).filter(
            id => tagMappings[id] && tagMappings[id].length > 0
          )
      ...
      updateCachedData() {
        ...
        if (this.cachedMindMaps.length > 0) {
          ...
          if (
            currentStoreMindMaps.length !== this.cachedMindMaps.length ||
            (this.cachedMindMaps.length > 0 &&
              currentStoreMindMaps[0]?.id !== this.cachedMindMaps[0]?.id)
          ) {
      ...
      // 注册导图管理页面的快捷键
      registerMindmapManagerShortcuts() {
        const highPriorityHandler = event => {
          ...
          if (
            ...
            event.stopImmediatePropagation() // 阻止事件继续传播到屏蔽器
            this.backToEditor()
            return false
          }
        }

        const resetAltHandler = event => {
          if (event.key === 'Alt') {
            this.altKeyPressed = false
          }
        }

        // 使用capture=true确保在屏蔽器之前执行
        document.addEventListener('keydown', highPriorityHandler, true)
        document.addEventListener('keyup', resetAltHandler, true)

        // 保存引用以便清理
        this.highPriorityHandler = highPriorityHandler
        this.resetAltHandler = resetAltHandler
      },

      // 清理导图管理页面的快捷键
      unregisterMindmapManagerShortcuts() {
        // 移除高优先级监听器
        if (this.highPriorityHandler) {
          document.removeEventListener(
            'keydown',
            this.highPriorityHandler,
            true
          )
          this.highPriorityHandler = null
        }

        if (this.resetAltHandler) {
          document.removeEventListener('keyup', this.resetAltHandler, true)
          this.resetAltHandler = null
        }
      ...
      handleTagEdit(tagId, tagData) {
        try {
          ...
          if (this.currentUser) {
            tagApi
              .updateTag(this.currentUser.id, tagId, tagData)
              .then(() => {})
              .catch(error => {
      ...
      async handleTagDelete(tagId) {
        try {
          ...
          this.cachedMindMapTagMapping = {
            ...TagCacheManager.getMindMapTagIds()
          }
          this.cachedUserTags = { ...TagCacheManager.getUserTags() }
      ...
      async handleMindmapAddTag(data) {
        ...
        try {
          ...
          const currentTags =
            TagCacheManager.getMindMapTagIds()[mindmapId] || []
      ...
      updateCategoryStats(action) {
        try {
          // 查找已分类和未分类的DOM元素
          const categorizedElement = document.querySelector(
            '[data-stat-type="categorized"] .stat-number'
          )
          const uncategorizedElement = document.querySelector(
            '[data-stat-type="uncategorized"] .stat-number'
          )
          ...
          const uncategorizedMatch =
            uncategorizedElement.textContent.match(/(\d+)/)

          let categorizedCount = categorizedMatch
            ? parseInt(categorizedMatch[1])
            : 0
          let uncategorizedCount = uncategorizedMatch
            ? parseInt(uncategorizedMatch[1])
            : 0
      ...
      getMindmapTagNames(mindmapId) {
        ...
        return tagIds
          .map(tagId => this.userTags[tagId]?.name || '')
          .filter(Boolean)
      },
