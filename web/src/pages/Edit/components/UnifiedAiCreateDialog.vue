<template>
  <div>
    ...
    <div v-show="aiLoading" class="ai-loading-overlay">
      ...
      <el-button type="warning" class="btn" @click="stopGenerate">
        停止生成
      </el-button>
    </div>
    ...
    methods: {
      ...
      async startActualGeneration() {
        ...
        try {
          ...
          // 显示开始生成的消息
          this.$message.info(
            `开始使用 ${this.currentProviderName} 生成思维导图...`
          )
      ...
      async handleSaveAndApply() {
        ...
        const originalData = JSON.parse(
          JSON.stringify(this.mindMap.getData(true))
        ) // 深拷贝原始数据
      ...
      initDragFunctionality() {
        ...
        this.initDragForDialog(
          '.draggable-unified-ai-create-dialog',
          'AI创建思维导图'
        )
      ...
      cleanupDragEvents() {
        if (this.dragHandler) {
          this.dragHandler.element.removeEventListener(
            'mousedown',
            this.dragHandler.mousedownHandler
          )
          this.dragHandler = null
        }

        if (this.saveConfirmDragHandler) {
          this.saveConfirmDragHandler.element.removeEventListener(
            'mousedown',
            this.saveConfirmDragHandler.mousedownHandler
          )
