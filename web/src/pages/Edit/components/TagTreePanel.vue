<template>
  <div class="tagTreePanel" :class="{ isDark: isDark }">
    ...
    <div class="tag-tree-container">
      <div class="tree-content">
        ...
        <template v-for="(tag, index) in sortedTags">
          ...
          <div
            v-if="
              tag.isPublic && (index === 0 || !sortedTags[index - 1].isPublic)
            "
            ...
            <div v-if="!tag.isPublic" class="tag-actions" @click.stop>
              <el-dropdown trigger="click" @command="handleTagAction">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    :command="{ action: 'edit', tagId: tag.id }"
                  >
                    <i class="el-icon-edit"></i>
                    编辑
                  </el-dropdown-item>
                  <el-dropdown-item
                    :command="{ action: 'delete', tagId: tag.id }"
                  >
    ...
    <el-dialog
      ...
      :close-on-press-escape="true"
      :modal-append-to-body="false"
      custom-class="delete-tag-confirm-dialog"
      :class="{ isDark: isDark }"
    >
      <div class="confirm-content">
        <div class="confirm-icon">
          <i
            ...
            style="color: #f56c6c; font-size: 48px"
          ></i>
        </div>
        <p class="confirm-text">
          <template v-if="deleteTagMindmapCount > 0">
            ...
            <span class="warning-text">
              这将影响
              <strong>{{ deleteTagMindmapCount }}</strong>
              个思维导图。
            </span>
          </template>
          <template v-else>
    ...
    methods: {
      ...
      handleDrop(event, tagId) {
        ...
        try {
          ...
          if (dragData.type === 'mindmap') {
            this.addTagToMindmap(
              dragData.mindmapId,
              tagId,
              dragData.mindmapTitle
            )
          }
        } catch (error) {
      ...
      async refreshTags() {
        ...
        try {
          ...
          // 1. 从数据库获取思维导图元数据
          const mindMapsData = await this.$store.dispatch(
            'getUserMindMaps',
            currentUser.id
          )
          ...
          const userTagsData = await tagApi.getUserTagsFromDatabase(
            currentUser.id
          )

          // 3. 从数据库获取思维导图与标签的关联数据
          const tagRelationsData = await tagApi.getMindMapTagRelations(
            currentUser.id
          )
