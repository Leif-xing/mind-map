<template>
  <div>
    ...
    >
      <div class="tipBox">
        ...
        <p>
          ...
          <a href="https://pan.baidu.com/s/1huasEbKsGNH2Af68dvWiOg?pwd=3bp3">
            {{ $t('ai.baiduNetdisk') }}
          </a>
          、
    ...
    <!-- AI续写专用加载动画遮罩层 -->
    <div class="aiPartCreatingMask" v-show="aiPartCreating">
      ...
      <el-button type="danger" class="btn" @click="stopPartCreate">
        停止续写
      </el-button>
    </div>
      ...
      <div class="confirm-content">
        ...
        <div class="confirm-text">
          ...
          <p class="current-title">
            当前思维导图：{{ currentMindMapTitle || '未命名思维导图' }}
          </p>
  ...
  export default {
    ...
    computed: {
      ...
      aiConfig() {
        const currentProvider =
          this.aiSystem.providers[this.aiSystem.currentProvider]
    ...
    methods: {
      ...
      async testConnect() {
        ...
        if (isDeployed) {
          // 确保使用HTTPS
          const secureApi = currentProvider.api.replace(
            /^http:\/\//,
            'https://'
          )
          ...
          try {
            ...
            if (response.ok) {
              ...
              this.$message.error(
                `${this.$t('ai.connectFailed')} (${response.status}): ${errorText}`
              )
            }
          } catch (error) {
          ...
          try {
            const response = await fetch(
              `http://localhost:${config.port}/ai/test`,
              {
                method: 'GET'
              }
            )
      ...
      async startActualAiCreate() {
        ...
        try {
          ...
          const currentConfig = await this.$store.dispatch(
            'fetchUserCurrentAiConfig',
            currentUserId
          )
          if (!currentConfig) {
      ...
      startTimer() {
        ...
        this.timerInterval = setInterval(() => {
          if (this.aiCreatingMaskVisible) {
            // 只有在遮罩显示时才计时
            const now = new Date()
            this.aiCreateElapsedTime = Math.floor(
              (now - this.aiCreateStartTime) / 1000
            )
          } else {
            // 如果遮罩已隐藏，停止计时器
            this.stopTimer()
      ...
      startPartTimer() {
        ...
        this.partTimerInterval = setInterval(() => {
          if (this.aiPartCreating) {
            // 只有在续写遮罩显示时才计时
            const now = new Date()
            this.aiPartElapsedTime = Math.floor(
              (now - this.aiPartStartTime) / 1000
            )
          } else {
            // 如果续写遮罩已隐藏，停止计时器
            this.stopPartTimer()
      ...
      fixParentChildTextMerge(node) {
        ...
        const childTexts = node.children
          .map(child => getPlainText(child.data.text))
          .filter(Boolean)
        ...
        if (childTexts.length > 0) {
          ...
          if (
            parentWithoutSpaces.includes(childrenJoined) ||
            childrenJoined.includes(parentWithoutSpaces)
          ) {
            ...
            if (cleanedParent.length > 0) {
              node.data.text = node.data.richText
                ? `<p>${cleanedParent}</p>`
                : cleanedParent
            } else {
      ...
      initCreateDialogDrag() {
        // 尝试多种选择器
        let dialogHeaderEl = document.querySelector(
          '.draggable-ai-create-dialog .el-dialog__header'
        )
        let dragDom = document.querySelector(
          '.draggable-ai-create-dialog .el-dialog'
        )

        // 如果custom-class找不到，尝试原始class
        if (!dialogHeaderEl || !dragDom) {
          dialogHeaderEl = document.querySelector(
        ...
        const mousedownHandler = e => {
          // 只有点击标题栏才触发拖拽
          if (
            e.target !== dialogHeaderEl &&
            !dialogHeaderEl.contains(e.target)
          ) {
            return
          }
      ...
      initPartDialogDrag() {
        // 尝试多种选择器
        let dialogHeaderEl = document.querySelector(
          '.draggable-ai-create-part-dialog .el-dialog__header'
        )
        let dragDom = document.querySelector(
          '.draggable-ai-create-part-dialog .el-dialog'
        )
        ...
        if (!dialogHeaderEl || !dragDom) {
          ...
          for (let dialog of allDialogs) {
            ...
            if (
              title &&
              (title.textContent.includes('续写') ||
                title.textContent.includes('AI续写'))
            ) {
              dragDom = dialog
      ...
      cleanupCreateDialogDragEvents() {
        if (this.createDialogDragHandler) {
          this.createDialogDragHandler.element.removeEventListener(
            'mousedown',
            this.createDialogDragHandler.mousedownHandler
          )
      ...
      cleanupPartDialogDragEvents() {
        if (this.partDialogDragHandler) {
          this.partDialogDragHandler.element.removeEventListener(
            'mousedown',
            this.partDialogDragHandler.mousedownHandler
          )
...
<style lang="less" scoped>
  ...
  // 使用最高优先级的深度选择器
  /deep/
    .createDialog.draggable-ai-create-part-dialog
    .el-textarea
    .el-textarea__inner {
    background-color: #f5f7fa !important; /* 浅灰色背景 */
    border-color: #dcdfe6 !important;
