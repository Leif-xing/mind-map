        ...
        >
          ...
          <el-table-column label="操作" width="200">
            <template slot-scope="scope">
              ...
              <el-button size="mini" @click="toggleConfigStatus(scope.row)">
          ...
          >
            <div class="modelInfo">
              <h3 class="providerName">
                {{ config.provider_name || config.providerName }}
              </h3>
              <p class="modelName">
                模型: {{ config.model_name || config.modelName }}
              </p>
              <el-tag
      ...
      >
        ...
        <el-form-item label="API密钥" prop="apiKey">
          <el-input
            ...
            show-password
            :placeholder="
              editingConfig ? '留空则不修改API密钥' : '请输入API密钥'
            "
          ></el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      selectedConfigValid() {
        const config = this.availableConfigs.find(
          c => c.id === this.selectedConfig
        )
    ...
    methods: {
              ...
              setTimeout(async () => {
                try {
                  const updatedConfigs =
                    await this.fetchAvailableAiConfigs(userId)
                  ...
                  if (
                    currentConfigId &&
                    this.availableConfigs.some(c => c.id === currentConfigId)
                  ) {
      ...
      editConfig(config) {
        ...
        this.configForm = {
          ...
          apiKey: '', // 不显示原有密钥
          isActive:
            config.is_active !== undefined ? config.is_active : config.isActive
        }
        this.showConfigDialog = true
      },
      ...
      async toggleConfigStatus(config) {
        try {
          const newStatus = !(config.is_active !== undefined
            ? config.is_active
            : config.isActive)
      ...
      async deleteConfig(config) {
        this.$confirm(
          `确定要删除 "${config.provider_name || config.providerName}" 配置吗？`,
          '删除确认',
          {
          ...
          .then(async () => {
            try {
              ...
              // 从本地列表中移除
              this.availableConfigs = this.availableConfigs.filter(
                c => c.id !== config.id
              )
      ...
      async saveConfig() {
        try {
          ...
          if (this.editingConfig) {
            ...
            await aiConfigApi.updateAiProviderConfig(
      ...
      initSelectionDialogDrag() {
        ...
        // 方法1：通过aria-label查找
        const dialogsByLabel = document.querySelectorAll(
          '[aria-label="选择AI服务"], [aria-label="AI服务管理"]'
        )
        ...
        if (!dialogHeaderEl || !dragDom) {
          ...
          for (let dialog of allDialogs) {
            ...
            if (
              titleEl &&
              (titleEl.textContent.includes('选择AI服务') ||
                titleEl.textContent.includes('AI服务管理'))
            ) {
              if (
                dialog.style.display !== 'none' &&
                dialog.offsetParent !== null
        ...
        if (!dialogHeaderEl || !dragDom) {
          ...
          console.log(
            '可见的对话框数量:',
            document.querySelectorAll('.el-dialog').length
          )
          console.log(
            'draggable-ai-selection-dialog元素数量:',
            document.querySelectorAll('.draggable-ai-selection-dialog').length
          )
          return
        }

        console.log(
          'AI选择对话框拖拽初始化成功，找到对话框:',
          dragDom.className
        )
        ...
        const mousedownHandler = e => {
          ...
          // 避免与关闭按钮的点击事件冲突
          if (
            e.target.classList.contains('el-dialog__headerbtn') ||
      ...
      cleanupSelectionDialogDragEvents() {
        if (this.selectionDialogDragHandler) {
          this.selectionDialogDragHandler.element.removeEventListener(
            'mousedown',
            this.selectionDialogDragHandler.mousedownHandler
          )
