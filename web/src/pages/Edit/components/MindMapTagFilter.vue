...
export default {
  ...
  computed: {
    ...
    selectedTagFiltersData() {
      return this.selectedTagFilters.map(tagId => 
    ...
    getFilteredMindMaps() {
      ...
      return this.mindMaps.filter(mindMap => {
        ...
        if (this.filterMode === 'all') {
          // 包含所有选中的标签
          return this.selectedTagFilters.every(tagId => mindMapTagIds.includes(tagId))
        } else {
          // 包含任一选中的标签
          return this.selectedTagFilters.some(tagId => mindMapTagIds.includes(tagId))
        }
      })
    },
