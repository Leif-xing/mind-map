<template>
  <div class="mindmapCards" :class="{ isDark: isDark }">
    ...
    <el-dialog
      ...
      :close-on-press-escape="true"
      :modal-append-to-body="false"
      custom-class="remove-tag-confirm-dialog"
      :class="{ isDark: isDark }"
    >
      <div class="confirm-content">
        ...
        <p class="confirm-text">
          确定要从思维导图 "
          <strong>{{ removeTagMindmapTitle }}</strong>
          " 中移除标签 "
          <strong>{{ removeTagName }}</strong>
          " 吗？
        </p>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleRemoveTagsHover(event) {
        ...
        if (this.contextMenuMindmap) {
          ...
          const cardElement = document.querySelector(
            `[data-mindmap-id="${this.contextMenuMindmap.id}"]`
          )
          ...
          if (cardElement) {
            ...
            setTimeout(() => {
              // 从DOM获取实际存在的标签（包括隐藏的）
              const tagElements = cardElement.querySelectorAll(
                '.card-tags .el-tag:not(.more-tags)'
              )
            ...
            // 回退方案：从缓存获取
            const tags = TagCacheManager.getMindMapTags(
              this.contextMenuMindmap.id
            )
      ...
      async handleDeleteMindmap() {
        ...
        try {
          await this.$confirm(
            `确定要删除思维导图 "${this.getMindmapTitle(mindmap)}" 吗？`,
            '删除确认',
            {
      ...
      async removeTagFromMindMap(mindMapId, tagId, userId) {
        try {
          ...
          // 调用API移除标签关联
          const result = await tagApi.removeTagFromMindMapOptimized(
            userId,
            mindMapId,
            tagId
          )
        ...
        try {
          ...
          try {
            ...
            const tagCache = JSON.parse(
              localStorage.getItem(tagCacheKey) || '{}'
            )
      ...
      findMindmapCardElement(mindmapId) {
        ...
        const cardElement = document.querySelector(
          `.mindmap-card[data-mindmap-id="${mindmapId}"]`
        )
      ...
      addTagToCardDOM(cardElement, tagId) {
        ...
        // 检查是否需要显示更多标签的提示
        const visibleTags = tagsContainer.querySelectorAll(
          '.el-tag:not(.more-tags)'
        )
      ...
      handleMoreTagsDisplay(tagsContainer, newTagElement) {
        ...
        newMoreTagEl.className =
          'el-tag el-tag--mini el-tag--info el-tag--light more-tags'
      ...
      removeTagFromCardDOM(cardElement, tagId) {
        ...
        // 获取思维导图ID
        const mindMapId =
          cardElement.getAttribute('data-mindmap-id') ||
          cardElement
            .closest('[data-mindmap-id]')
      ...
      updateMoreTagsCount(tagsContainer, mindMapId) {
        ...
        if (moreTagEl && mindMapId) {
          // 从缓存获取实际标签总数
          const actualTagsCount =
            TagCacheManager.getMindMapTags(mindMapId).length
          ...
          if (moreCount > 0) {
            ...
            moreTagEl.remove()
            // 显示一个之前隐藏的标签
            const firstHidden = tagsContainer.querySelector(
      ...
      handleTagRemoved(data) {
        ...
        this.$nextTick(() => {
          // 🔍 检查并修复DOM与缓存的一致性
          const wasFixed = this.checkAndFixTagConsistency(
            mindmapId,
            '标签移除事件处理'
          )
      ...
      adjustTagDisplayAfterRemoval(tagsContainer, mindMapId) {
        try {
          if (!mindMapId) {
            ...
            mindMapId = mindMapCard
              ? mindMapCard.getAttribute('data-mindmap-id')
              : null
          }
          ...
          const visibleTags = tagsContainer.querySelectorAll(
            '.el-tag:not(.more-tags):not([style*="display: none"])'
          )
          const visibleTagsCount = visibleTags.length // a

          // 获取隐藏标签元素（display为none的标签）
          const hiddenTags = tagsContainer.querySelectorAll(
            '.el-tag:not(.more-tags)[style*="display: none"]'
          )
          const hiddenTagsCount = hiddenTags.length // b

          // 获取DOM中所有标签元素的data-tag-id总数量
          const allTagElements = tagsContainer.querySelectorAll(
            '.el-tag:not(.more-tags)[data-tag-id]'
          )
        ...
        try {
          ...
          this.sortedMindmaps.forEach((mindmap, index) => {
            try {
              ...
              const wasFixed = this.checkAndFixTagConsistency(
                mindmap.id,
                `${triggerContext}-批量检查第${index + 1}项`
              )
          ...
          if (!tagsContainer) {
            ...
            try {
              ...
              if (cacheTagsData.length > 0) {
                // 创建标签容器并重新构建
                this.createTagsContainerAndRebuild(
                  cardElement,
                  mindMapId,
                  cacheTagsData
                )
      ...
      clearTagDisplayOverride(mindmapId, tagId) {
        if (
          this.tagDisplayOverrides[mindmapId] &&
          this.tagDisplayOverrides[mindmapId][tagId]
        ) {
        ...
        try {
          ...
          if (!cardElement) {
            ...
            cardElement = document.querySelector(
              `.mindmap-card[data-id="${mindmapId}"]`
            )
          }

          if (!cardElement) {
            const allCards = document.querySelectorAll(
              '.mindmap-card, [class*="mindmap"], [class*="card"]'
            )
            return
          }
          ...
          let tagsContainer = cardElement.querySelector('.card-tags')
          if (!tagsContainer) {
            tagsContainer = cardElement.querySelector(
              '.tags, .tag-list, [class*="tag"]'
            )
          }
      ...
      adjustTagsDisplay(tagsContainer) {
        try {
          ...
          const mindMapId = tagsContainer
            .closest('[data-mindmap-id]')
            ?.getAttribute('data-mindmap-id')
          const actualTagsCount = mindMapId
            ? TagCacheManager.getMindMapTags(mindMapId).length
            : 0

          // 获取DOM中的标签元素（用于显示操作）
          const allTags = Array.from(
            tagsContainer.querySelectorAll('.el-tag:not(.more-tags)')
          )
          let moreTagEl = tagsContainer.querySelector('.more-tags')
          ...
          if (actualTagsCount <= 3) {
            ...
            // 检查当前可见标签数量
            const currentVisibleTags = allTags.filter(
              tag => tag.style.display !== 'none'
            )
            const currentHiddenTags = allTags.filter(
              tag => tag.style.display === 'none'
            )

            // 如果可见标签不足3个，从隐藏标签中补充
            if (currentVisibleTags.length < 3 && currentHiddenTags.length > 0) {
              const needToShow = Math.min(
                3 - currentVisibleTags.length,
                currentHiddenTags.length
              )
            ...
            // 如果可见标签超过3个，隐藏多余的
            const nowVisibleTags = allTags.filter(
              tag => tag.style.display !== 'none'
            )
            ...
            if (moreCount > 0) {
              const newMoreTagEl = document.createElement('span')
              newMoreTagEl.className =
                'el-tag el-tag--mini el-tag--info more-tags'
              newMoreTagEl.textContent = `+${moreCount}`
              tagsContainer.appendChild(newMoreTagEl)
            }
          }
