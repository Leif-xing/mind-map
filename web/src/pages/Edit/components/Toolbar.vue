    ...
    >
      <div class="confirm-content">
        <p class="confirm-text">
          检测到当前思维导图"
          <strong>{{ currentMindMapTitleForNew }}</strong>
          "已发生变化，是否需要保存？
        </p>
      </div>
    ...
    methods: {
      ...
      handleKeyDown(event) {
        ...
        if (event.shiftKey && event.key.toLowerCase() === 'c') {
          ...
          if (this.$mindMap && this.$mindMap.getSelectedNodeList().length > 0) {
            ...
            this.$bus.$emit(
              'showNodeNote',
              this.$mindMap.getSelectedNodeList()[0]
            )
          }
        }
      },
      ...
      async handleLoadMindMap(mindMap) {
        ...
        try {
          ...
          // 2. 从缓存中获取目标思维导图的数据
          const contentToLoad = await this.getMindMapDataFromCache(
            mindMapToLoad.id
          )
      ...
      async preloadAiConfigs() {
        try {
          ...
          const aiConfigs = await this.$store.dispatch(
      ...
      async batchDeleteMindMaps() {
        ...
        try {
          ...
          this.$message.success(
            `成功删除 ${this.selectedMindMaps.length} 个思维导图`
          )
          ...
          // 获取所有缓存的ID
          const allCacheIds = Object.keys(
            this.$store.state.mindMapCacheManager?.cache || {}
          )
      ...
      async deleteAllMindMaps() {
        ...
        try {
          ...
          this.$message.success(
            `成功删除所有 ${this.mindMaps.length} 个思维导图`
          )
      ...
      showNewMindMapSaveConfirmDialog() {
        ...
        this.$nextTick(() => {
          setTimeout(() => {
            this.initDragForNewMindMapDialog(
              '.draggable-new-mindmap-confirm-dialog',
              '温馨提示'
            )
          }, 100)
        })
      ...
      getCurrentMindMapTitleForNew() {
        try {
          ...
          if (
            currentMindMap &&
            currentMindMap.renderer &&
            currentMindMap.renderer.root
          ) {
            const rootData = currentMindMap.renderer.root.getData()
            if (rootData && rootData.text) {
              // 移除HTML标签，获取纯文本
              this.currentMindMapTitleForNew = rootData.text
                .replace(/<[^>]*>/g, '')
                .trim()
            } else {
        ...
        try {
          ...
          const newMindMapData = {
            ...
            config: {
              // 保持当前的一些基本配置
              enableFreeDrag:
                this.$store.state.localConfig.enableFreeDrag || false,
      ...
      async handleSaveAndNewMindMap() {
        ...
        const originalData = JSON.parse(
          JSON.stringify(currentMindMap.getData(true))
        ) // 深拷贝原始数据
      ...
      cleanupNewMindMapSaveConfirmDragEvents() {
        if (this.newMindMapSaveConfirmDragHandler) {
          this.newMindMapSaveConfirmDragHandler.element.removeEventListener(
            'mousedown',
            this.newMindMapSaveConfirmDragHandler.mousedownHandler
          )
